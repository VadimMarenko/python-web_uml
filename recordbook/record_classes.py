...
class Record:
    ...
    def change_phone(self, old_phone: Phone, new_phone: Phone) -> str:
        for phones in self.phones:
            if str(old_phone) == str(phones):
                ...
                return (
                    f"Phone {old_phone} change to {new_phone} for {self.name} contact "
                )
    ...
    def __str__(self):
        return "{}{}{}{}{}".format(
            ...
            else "Birthday: No birthday date\n",
        )
    ...
    def check_dublicate_phone(self, search_phone: str) -> bool:
        result = list(
            map(
                lambda phone: any(phone.value == search_phone),
                self.data[self.name.value].phones,
            )
        )
