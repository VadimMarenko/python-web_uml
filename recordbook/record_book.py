...
def func_add_rec(*args):
    if not args[0].capitalize() in book.keys():
        ...
        return "The person is already in database"
...
def add_phone(*args):
    ...
    if args and (len(args) >= 2):
        ...
        return f"Expected 2 arguments, but {len(args)} was given.\nHer's an example >> add phone Mike +380509998877"
...
@input_error
def add_email(*args):
    """Додавання email до запису контакту.
...
def add_address(*args):
    """Збереження адреси запису контакту.

    адресу можна вводити як завгодно.
...
def add_birthday(*args):
    """Запис дня народження, birthday контакта.
...
def func_show_all(args) -> str:
    """Посторінкове виведення всіх записів книги контактів.
...
def func_phone(*args):
    ...
    if name in book.keys():
        if args:
            ...
            return (
                f"Expected 1 argument, but 0 was given.\nHer's an example >> phone Name"
            )
    else:
...
def func_get_day_birthday(*args):
    ...
    if args[0] == "":
        return f"Missed [bold red]Name[/bold red] of the person"

    if args and (count_prm >= 1):
        if "/" in args[0]:  # Example >> birthday /365
            count_day = int(re.sub("\/", "", args[0]))
            if not count_day > 0:
                return f"Enter the number of days greater than zero"
            return book.get_list_birthday(count_day)

        else:  # Example >> birthday Mike
            name = args[0].lower().capitalize()
            if name in book.keys():
                if book[name].birthday.value == "None":
                    return f"No [bold red]Birthday[/bold red] for {name}"
                return book[name].days_to_birthday()
            else:
                return f"The [bold red]{name}[/bold red] isn't in a database"
    else:
...
def delete(*args):
    ...
    if args[0].lower() == "name":
        ...
        rec.remove_birthday()
        return (
            f"{args[1].capitalize()}'s birthday has been removed from the contact list"
        )
...
def change(*args):
    if args[1].capitalize() in book.keys():
        ...
        raise BirthdayException(f"Name {args[1].capitalize()} isn't in datebase")
...
def func_search(*args):
    ...
    if args[0] == "":
        return f"[bold yellow]Enter search information[/bold yellow]"
    lst_result = []
    rec_str = ""
    if args and (count_prm >= 1):
